...
class SafeCheckoutApi
{
     ...
     * @param  string $ulid Offer identifier (required)
     * @param  \Tripartie\SafeCheckout\Model\OfferSoloWrite $offerSoloWrite The new Offer resource (required)
     ...
     * @return \Tripartie\SafeCheckout\Model\OfferPostCreationRead|\Tripartie\SafeCheckout\Model\InvalidQueryError|\Tripartie\SafeCheckout\Model\UnprocessableEntity|\Tripartie\SafeCheckout\Model\AuthError|\Tripartie\SafeCheckout\Model\AccessError|\Tripartie\SafeCheckout\Model\RateLimitError|\Tripartie\SafeCheckout\Model\GenericError
     */
    public function apiUsersIdoffersPost($id, $ulid, $offerSoloWrite, string $contentType = self::contentTypes['apiUsersIdoffersPost'][0])
    {
        list($response) = $this->apiUsersIdoffersPostWithHttpInfo($id, $ulid, $offerSoloWrite, $contentType);
        return $response;
    }
     ...
     * @return array of \Tripartie\SafeCheckout\Model\OfferPostCreationRead|\Tripartie\SafeCheckout\Model\InvalidQueryError|\Tripartie\SafeCheckout\Model\UnprocessableEntity|\Tripartie\SafeCheckout\Model\AuthError|\Tripartie\SafeCheckout\Model\AccessError|\Tripartie\SafeCheckout\Model\RateLimitError|\Tripartie\SafeCheckout\Model\GenericError, HTTP status code, HTTP response headers (array of strings)
     */
    public function apiUsersIdoffersPostWithHttpInfo($id, $ulid, $offerSoloWrite, string $contentType = self::contentTypes['apiUsersIdoffersPost'][0])
    {
        $request = $this->apiUsersIdoffersPostRequest($id, $ulid, $offerSoloWrite, $contentType);
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiUsersIdoffersPostAsync($id, $ulid, $offerSoloWrite, string $contentType = self::contentTypes['apiUsersIdoffersPost'][0])
    {
        return $this->apiUsersIdoffersPostAsyncWithHttpInfo($id, $ulid, $offerSoloWrite, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function apiUsersIdoffersPostAsyncWithHttpInfo($id, $ulid, $offerSoloWrite, string $contentType = self::contentTypes['apiUsersIdoffersPost'][0])
    {
        $returnType = '\Tripartie\SafeCheckout\Model\OfferPostCreationRead';
        $request = $this->apiUsersIdoffersPostRequest($id, $ulid, $offerSoloWrite, $contentType);
     ...
     * @return \GuzzleHttp\Psr7\Request
     */
    public function apiUsersIdoffersPostRequest($id, $ulid, $offerSoloWrite, string $contentType = self::contentTypes['apiUsersIdoffersPost'][0])
    {
        ...
        if ($offerSoloWrite === null || (is_array($offerSoloWrite) && count($offerSoloWrite) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $offerSoloWrite when calling apiUsersIdoffersPost'
            );
        }
        ...
        // for model (json/xml)
        if (isset($offerSoloWrite)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \GuzzleHttp\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($offerSoloWrite));
            } else {
                $httpBody = $offerSoloWrite;
            }
        } elseif (count($formParams) > 0) {
            if ($multipart) {
                $multipartContents = [];
